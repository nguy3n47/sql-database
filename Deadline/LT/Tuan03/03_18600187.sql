...
-- TRUY VẤN TUẦN 3
-- ĐỀ 2 - CÂU 2
SELECT DA.TENDA, PB.TENPHG, NV.HONV + SPACE(1) + NV.TENLOT + SPACE(1) + NV.TENNV HOTEN, PB.NG_NHANCHUC
FROM DEAN DA JOIN PHONGBAN PB ON DA.PHONG = PB.MAPHG JOIN NHANVIEN NV ON PB.MAPHG=NV.PHG
WHERE PB.TRPHG = NV.MANV

-- ĐỀ 2 - CÂU 3
SELECT DA.TENDA, COUNT(DA.MADA)
FROM DEAN DA JOIN CONGVIEC CV ON DA.MADA = CV.MADA
GROUP BY DA.MADA, DA.TENDA

-- ĐỀ 2 - CÂU 4
SELECT PB.TENPHG, MAX(NV.LUONG)
FROM NHANVIEN NV JOIN PHONGBAN PB ON NV.PHG = PB.MAPHG
GROUP BY NV.PHG, PB.TENPHG
HAVING MAX(NV.LUONG) > 30000

-- ĐỀ 3 - CÂU 2
SELECT NV.HONV + ' ' + NV.TENLOT + ' ' + NV.TENNV NHANVIEN, TP.HONV + ' ' + TP.TENLOT + ' ' + TP.TENNV TRUONGPHONG
FROM NHANVIEN NV, NHANVIEN TP
WHERE NV.MA_NQL = TP.MANV

-- ĐỀ 3 - CÂU 3
SELECT PB.MAPHG, PB.TENPHG, TP.TRUONGPHONG, SLNV.SL
FROM PHONGBAN PB JOIN (SELECT NV.PHG, COUNT(*) SL
						FROM NHANVIEN NV
						GROUP BY PHG) SLNV ON PB.MAPHG = SLNV.PHG
				 JOIN (SELECT NV.MANV, NV.HONV + ' ' + NV.TENLOT + ' ' + NV.TENNV TRUONGPHONG
						FROM NHANVIEN NV) TP ON TP.MANV = PB.TRPHG

-- ĐỀ 3 - CÂU 4
SELECT MAPHG, TENPHG , COUNT (MADA) 'SL DEAN'
FROM PHONGBAN PB JOIN DEAN DA ON PB.MAPHG = DA.PHONG
GROUP BY MAPHG, TENPHG
HAVING COUNT(MADA) >=3 AND COUNT (MADA) <=5

-- ĐỀ 4 - CÂU 2
SELECT NV.HONV + ' ' + NV.TENLOT + ' ' + NV.TENNV HOTEN -- NV.PHG, DA.TENDA
FROM NHANVIEN NV JOIN DEAN DA ON NV.PHG = DA.PHONG
WHERE DA.TENDA = N'SẢN PHẨM X'

-- ĐỀ 4 - CÂU 3
SELECT DA.MADA, DA.TENDA, COUNT (*) SLNV
FROM DEAN DA JOIN PHANCONG PC ON DA.MADA = PC.MADA
GROUP BY DA.MADA, DA.TENDA

-- ĐỀ 4 - CÂU 4
SELECT PB.MAPHG, PB.TENPHG, COUNT (*) 'SL DIADIEM'
FROM PHONGBAN PB JOIN DIADIEM_PHG DD ON PB.MAPHG = DD.MAPHG
GROUP BY PB.MAPHG, PB.TENPHG
HAVING COUNT (*) > 2

-- ĐỀ 5 - CÂU 2
SELECT DA.TENDA
FROM NHANVIEN NV JOIN ( SELECT PA.MA_NVIEN, PA.MADA
						FROM PHANCONG PA) PC ON NV.MANV = PC.MA_NVIEN 
				 JOIN ( SELECT DE.MADA, DE.TENDA
						FROM DEAN DE) DA ON PC.MADA = DA.MADA
WHERE NV.HONV + ' ' + NV.TENLOT + ' ' + NV.TENNV = N'ĐINH BÁ TIẾN'

-- ĐỀ 5 - CÂU 3
SELECT PB.MAPHG, PB.TENPHG, COUNT (*) 'SL DEAN'
FROM PHONGBAN PB JOIN DEAN DA ON PB.MAPHG = DA.PHONG
GROUP BY PB.MAPHG, PB.TENPHG
